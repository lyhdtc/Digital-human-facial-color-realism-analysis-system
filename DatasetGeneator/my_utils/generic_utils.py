...
def lyh_save_set_of_images(path, prefix, images, pic_number, show_prog_bar=False, name_list=None):
    os.makedirs(path, exist_ok=True)
    if len(images.shape) == 4:  # Color channels present
        if images.shape[-1] == 1:
            images = images[:, :, :, 0]
    ...
    cur_name = str(pic_number).zfill(5)
    imageio.imwrite(os.path.join(path, prefix + f'{cur_name}.png'), images[0])
